-- Create a separate schema for Simurgh to avoid conflicts
CREATE SCHEMA IF NOT EXISTS simurgh;

-- Set search path to include our schema
SET search_path TO simurgh, public;
...
CREATE INDEX IF NOT EXISTS idx_rfq_documents_status ON simurgh.rfq_documents(status);
CREATE INDEX IF NOT EXISTS idx_rfq_documents_rfq_number ON simurgh.rfq_documents(rfq_number);
CREATE INDEX IF NOT EXISTS idx_rfq_responses_status ON simurgh.rfq_responses(status);
CREATE INDEX IF NOT EXISTS idx_rfq_history_rfq_document_id ON simurgh.rfq_history(rfq_document_id);